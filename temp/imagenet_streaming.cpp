...
int main(int argc, char **argv)
{
    ...
    //****** Create a socket_server sensor thread ***************
    socket_server *soc_server_obj;
    pthread_mutex_t *mut2;
    mut2 = new pthread_mutex_t;
    pthread_mutex_init(mut2, NULL);
    soc_server_obj = new socket_server(mut2);
    const int send_bytes = 1000;
    soc_server_obj->set_image_data_size(send_bytes); //Tesar med 12 byte
    for (int i = 0; i < send_bytes; i++)
    {
        soc_server_obj->socket_send[i] = i + 20; //Test
    }
    // Start the thread, send it the this pointer (points to this class
    // instance)
    soc_server_obj->Start();
    //*********************************************************
    
    //soc_server_obj->debug();

    cv::Mat opencv_frame;
    int nr_of_frame_bytes = 0;
    int total_send_bytes = 0;
    int size_of_opencv_frame = 0;
    
    //set up all pointers for Opencv
    uint8_t *opencv_uint8t_zero_ptr = opencv_frame.ptr<uint8_t>(0);
    uint8_t *opencv_uint8t_indx_ptr = opencv_frame.ptr<uint8_t>(0);
    ...
    printf("======== With streaming images Cat ============\n");
    ...
    if (!signal_recieved)
    {
        ...
        while (!signal_recieved)
        {
            ...
            if (use_OPENCV == 1)
            {
                //Copy over GPU frame to CPU and OpenCV
                ...
                opencv_f_zero_ptr = opencv_frame.ptr<float>(0);
                opencv_f_indx_ptr = opencv_frame.ptr<float>(0);
                img_gpu_src_zero_ptr = (float3 *)&host_src[0];
                img_gpu_src_indx_ptr = (float3 *)&host_src[0];
                ...
                opencv_uint8t_zero_ptr = opencv_frame.ptr<uint8_t>(0);
                opencv_uint8t_indx_ptr = opencv_frame.ptr<uint8_t>(0);
                size_of_opencv_frame = opencv_frame.cols * opencv_frame.rows;
                nr_of_frame_bytes = size_of_opencv_frame * opencv_frame.channels();

                if (soc_server_obj->socket_send.size() != nr_of_frame_bytes && nr_of_frame_bytes <= (soc_server_obj->get_max_image_size()))
                {
                    //Set the correct size of the buffert
                    soc_server_obj->set_image_data_size(nr_of_frame_bytes); //
                    opencv_is_initialized = 1;
                }else{
                    opencv_is_initialized = 0;
                }
            ...
            if (use_OPENCV == 1)
            {
               // cv::imwrite(arr, opencv_frame);//Seg fault ?
